...
int knn_predict(unsigned char *input, int K,
    ...
    for(int index = 0; index < training_size; index++){
        ...
        char current_label = labels[index];
    ...
    for(int index3 = 0; index3 < K; index3++){
        // int cur_index = top_labels[index3] - '0';
        sample[(int)top_labels[index3]] = sample[(int)top_labels[index3]] + 1;
        }
    ...
    // int max_frequency = 0, current_frequency = 0, best_match = 0;
    // for(int i = 0; i < K; i++){
    //     current_frequency++;
    //     for(int j = i + 1; j < K; j++){
    ...
    //         best_match = top_labels[i];
    //     }
    //     else if((current_frequency == max_frequency) && (top_labels[i] < best_match)){
    //         best_match = top_labels[i];
    //     }
    //     current_frequency = 0;
